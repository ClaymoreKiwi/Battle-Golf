...
void Enemy::update()
{
	...
	CheckCollisionClub(player->GetGolfClub());
	for (auto bullet : player->AmmoTotal)
	{
		if(!bullet->inCollision && bullet->used && !e_knockedOut)
		{
			if (bullet->CheckCollision(e_drawDest, bullet->GetCollider(), e_health, e_speed))
			{
				player->SetSwings();
			}
		}
...
bool Enemy::CheckCollisionClub(SDL_Rect* other)
{
	if (!e_knockedOut && player->GetClub()->isThrown && !player->GetClub()->collided)
	{
		SDL_Rect check = { other->x - e_camera->x, other->y - e_camera->y, other->w, other->h };
		if (SDL_HasIntersection(&check, &e_drawDest))
		{
			//std::cout << "Enemy: Collision with club" << std::endl;
			e_health -= 5;
			player->SetGolfClubState();
			return true;
		}
	}
	return false;

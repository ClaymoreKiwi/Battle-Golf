...
std::vector<Node> aStar(const TileMap& theMap, const Node& enemy, const Node& dest) 
{
	...
	while (!openList.empty()) {
		...
		//diaginal Up Right
		if (isValid(theMap, current.x + 1, current.y - 1) && !closedList[current.y - 1][current.x + 1]) {
			float gNew = nodeDetails[current.y][current.x].gCost + 1.4f;
			float hNew = calculateH(current.x + 1, current.y - 1, dest);
			float fNew = gNew + hNew;

			if (nodeDetails[current.y - 1][current.x + 1].fCost == FLT_MAX || nodeDetails[current.y -1][current.x + 1].fCost > fNew) {
				// Update the details of this neighbour node
				nodeDetails[current.y - 1][current.x + 1].fCost = fNew;
				nodeDetails[current.y - 1][current.x + 1].gCost = gNew;
				nodeDetails[current.y - 1][current.x + 1].hCost = hNew;
				nodeDetails[current.y - 1][current.x + 1].parentX = current.x;
				nodeDetails[current.y - 1][current.x + 1].parentY = current.y;

				openList.push_back(nodeDetails[current.y -1][current.x + 1]);
			}
		}
